...
void mouseDragged() {
  ...
  if (proper_clicked) {
    boolean bottom_hits = (coord_circle_pairs[row_to_change][0].y+dragged_y > screen_width);
    println("bottom_hits ", bottom_hits);
    boolean top_hits = (coord_circle_pairs[row_to_change][num_of_balls-1].y+dragged_y < 0);
    println("top_hits ", top_hits);
    boolean left_hits = (coord_circle_pairs[row_to_change][0].x+dragged_x < 0);
    println("left_hits ", left_hits);
    boolean right_hits = (coord_circle_pairs[row_to_change][num_of_balls-1].x+dragged_x > screen_length);
    println("right_hits ", right_hits);
    dragged_x = mouseX - pmouseX;
    dragged_y = mouseY - pmouseY;
    if (bottom_hits || top_hits || left_hits || right_hits) {
      if (bottom_hits) {
        dragged_y = screen_width - coord_circle_pairs[row_to_change][0].y;
      }
      else if (top_hits) {
        dragged_y = 0 - coord_circle_pairs[row_to_change][num_of_balls-1].y;
      }
      else if (left_hits) {
        dragged_x = 0 - coord_circle_pairs[row_to_change][0].x;
      }
      else if (right_hits) {
        dragged_x = screen_length - coord_circle_pairs[row_to_change][num_of_balls-1].x;
      }
    }
...
void mouseReleased() {
  ...
  for (int row = 0; row < row_num; row++) {
    possible_ranges[row][0] = coord_circle_pairs[row][0].x;
    possible_ranges[row][1] = coord_circle_pairs[row][0].y - CIRCLE_WIDTH;
    possible_ranges[row][2] = coord_circle_pairs[row][num_of_balls-1].x;
    possible_ranges[row][3] = coord_circle_pairs[row][num_of_balls-1].y;
  }
}
